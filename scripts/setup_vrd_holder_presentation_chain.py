#!/usr/bin/env python
"""Build a real Signify/KERIA GEDA -> LE VRD chain for W3C presentation E2E.
...
holder-presentation acceptance tests can consume. W3C artifact assembly is
driven by edge helpers outside core SignifyPy.
"""
...
except ImportError:  # pragma: no cover - operator environment guidance
    ensure_didwebs_setup = None
...
def main() -> None:
    ...
    manifest = {
        ...
        "holderWallet": {
            "name": le.name,
            "aid": le.aid,
            "passcode": le.passcode,
        },
        "sourceCredentialSaid": vrd_cred["said"],
        "holderPresentation": {
            "issuerName": qvi.name,
            "holderName": le.name,
            "sourceCredentialSaid": vrd_cred["said"],
    ...
    print(f"W3C issuer/QVI wallet alias: {qvi.name}", file=sys.stderr)
    print(f"Holder presentation source VRD credential SAID: {vrd_cred['said']}", file=sys.stderr)
...
def log(message: str) -> None:
    print(f"[setup-vrd-holder-presentation] {message}", file=sys.stderr, flush=True)
...
def wait_for_didwebs_ready(client: SignifyClient, name: str, aid: str, *, timeout: float) -> str:
    if ensure_didwebs_setup is None:
        raise RuntimeError(
            "did:webs setup orchestration moved out of SignifyPy. Install "
            "signifypy-did-webs from w3c-crosswalk before running this script."
        )

    setup = ensure_didwebs_setup(
        client,
        name,
